...
int GPX_GEN::generateSkeleton(char* buffer, int bufferSize, const char* creator, const char* trackName) {
    ...
    return 37; // there are 31 bytes between the space where new data can be inserted and the end
}
...
bool GPX_GEN::generateTrackpoint(char* buffer, int bufferSize, float latitude, float longitude, float elevation, int year, int month, int day, int hour, int minute, int second) {
    ...
    sprintf(buffer, "\t\t\t<trkpt lat=\"%f\" lon=\"%f\">\n\t\t\t\t<ele>%f</ele>\n\t\t\t\t<time>%d-%d-%dT%d:%d:%dZ</time>\n\t\t\t</trkpt>\n\n\t\t</trkseg>\n\t</trk>\n</gpx>", (double)latitude, (double)longitude, (double)elevation, year, month, day, hour, minute, second);
    return true;
}