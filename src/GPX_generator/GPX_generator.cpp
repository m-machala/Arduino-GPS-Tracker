...
int GPX_GEN::generateSkeleton(char* buffer, int bufferSize, char* creator, char* trackName) {
...
bool GPX_GEN::generateTrackpoint(char* buffer, int bufferSize, float latitude, float longitude, float elevation, int year, int month, int day, int hour, int minute, int second) {
    ...
    sprintf(buffer, "\t\t\t<trkpt lat=\"%f\" lon=\"%f\">\n\t\t\t\t<ele>%f</ele>\n\t\t\t\t<time>%d-%d-%dT%d:%d:%dZ</time>\n\t\t\t</trkpt>\n", (double)latitude, (double)longitude, (double)elevation, year, month, day, hour, minute, second);
    return true;
}